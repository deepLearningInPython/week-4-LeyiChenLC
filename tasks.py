...
tokens = [words.strip(".,!?") for words in text.split()]
...
def tokenize(string: str) -> list:
    tokens = [words.strip("!?@#$%^&*().,").lower() for words in string.split()]
...
# Your code here:
# -----------------------------------------------
word_frequencies = {words: tokens.count(words) for words in tokens}

# Expected output example: {'the': 2, 'quick': 1, ...}
print(word_frequencies)

# Modify the comprehension to include only words that appear more than once.
word_frequencies = {words: tokens.count(words) for words in tokens if tokens.count(words) > 1 }
# -----------------------------------------------
...
token_to_id = {token: idx for idx, token in enumerate(sorted(set(token.lower() for token in tokens)))}
...
def tokenize_and_encode(documents: list) -> list:
    ...
    for docs in documents:
        unique_tokens = tokenize(docs)
        encoded_doc = [token_to_id[token] for token in unique_tokens]
